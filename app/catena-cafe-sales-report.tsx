    ...
    <ScrollView style={styles.container}>
      ...
      {sales.map(s => (
        <View key={s.id} style={{ flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderColor: '#eee', paddingVertical: 8 }}>
          <Text style={{ flex: 1, textAlign: 'center' }}>{formatDate(s.createdAt)}</Text>
          <Text style={{ flex: 1, textAlign: 'center' }}>{s.status || ''}</Text>
          <Text style={{ flex: 1, textAlign: 'center' }}>{s.type || ''}</Text>
          <Text style={{ flex: 1, textAlign: 'center' }}>{s.cash || 0}</Text>
        </View>
      ))}
      <Button mode="contained" onPress={handleDownloadPDF} style={{ marginTop: 24, backgroundColor: '#43a047' }} loading={loading} disabled={loading || sales.length === 0}>
        Download PDF
      </Button>
