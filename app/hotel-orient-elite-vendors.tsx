  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const maintQ = query(collection(db, 'maintenanceLogs'), where('branchId', '==', 'orientElite'));
        ...
        // Fetch all vendors
        const vendorQ = query(collection(db, 'vendors'), where('branchId', '==', 'orientElite'));
        ...
        Alert.alert('Error', error.message || 'Failed to fetch vendor data.');
      }
      setLoading(false);
    };
    fetchData();
  ...
  const handleDownloadPDF = async (type: 'maintenance' | 'common') => {
    try {
      ...
      if (type === 'maintenance') {
        ...
        filename = 'maintenance_vendors.pdf';
      } else {
        html = generateCommonVendorsHTML();
        filename = 'common_vendors.pdf';
      }
