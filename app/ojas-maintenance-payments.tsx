...
export default function OjasMaintenancePayments() {
  ...
  async function fetchPayments() {
    ...
    try {
      const q = query(
        collection(db, 'ojasvendorpayments'),
  ...
  async function handleSave() {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'ojasvendorpayments', selected.id), { status });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <MaterialCommunityIcons name="wrench" size={28} color={DARK_BROWN} />
        ...
        <Card style={styles.actionCard}>
          <Card.Content>
            <TouchableOpacity 
              ...
              onPress={() => router.push('/ojas-all-vendor-payments')}
            >
